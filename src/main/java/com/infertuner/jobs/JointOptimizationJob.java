...
public class JointOptimizationJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 解析参数
        int parallelism = args.length > 0 ? Integer.parseInt(args[0]) : 2;        // p: 并行度/GPU数量
        int batchSize = args.length > 1 ? Integer.parseInt(args[1]) : 4;          // b: 每GPU批大小
        int batchesPerGpu = args.length > 2 ? Integer.parseInt(args[2]) : 6;      // 每GPU处理的批次数
        long interval = args.length > 3 ? Long.parseLong(args[3]) : 200;         // 请求间隔

        // 计算总请求数：确保每个GPU都能处理足够的完整批次
        int totalRequests = parallelism * batchSize * batchesPerGpu;

        // 超时配置：确保最后几个请求不会永久等待
        long maxWaitTimeMs = Math.max(batchSize * interval * 2, 3000);

        logger.info("🎯 p×b联合优化配置:");
        logger.info("  并行度(p): {} GPU", parallelism);
        logger.info("  批大小(b): {} 请求/批次", batchSize);
        logger.info("  每GPU批次数: {}", batchesPerGpu);
        logger.info("  总请求数: {} = {}×{}×{}", totalRequests, parallelism, batchSize, batchesPerGpu);
        logger.info("  请求间隔: {}ms", interval);
        logger.info("  超时保护: {}ms", maxWaitTimeMs);
        logger.info("  预期总批次: {}", parallelism * batchesPerGpu);
        ...
        config.setString("batches.per.gpu", String.valueOf(batchesPerGpu));
        ...
        responses.addSink(new JointOptimizationSink(experimentId, parallelism, batchSize))
                .name("Joint Optimization Performance Sink");
