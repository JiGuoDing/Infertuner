...
public class InferTunerTestingJob {
    ...
    public static void main(String[] args) {
        ...
        // 参数格式：--maxRequests 100 --parallelism 4 --batchsize 4 --baseInterval 2000 --enableLoadVariation true
        // 默认 100 个请求
        ...
        logger.info("启动参数: maxRequests={}, parallelism={}, batchsize={} baseInterval={}, enableLoadVariation={}", maxRequests, parallelism, batchsize, baseInterval, enableLoadVariation);
        ...
        /*
            * TODO 添加数据源
        */
        DataStream<InferenceRequest> requests = env.addSource(new CacheAwareRequestSource(maxRequests, baseInterval, enableLoadVariation)).name("Cache-aware Requests Source");

        /*
            * TODO 添加请求预处理算子(预测请求的推理时间)
        */
        DataStream<InferenceRequest> predicted_requests = requests.flatMap(new PredictingInferenceTimeProcessor()).name("Predicting Inference Time Operator");
        /*
            * TODO 添加请求推理算子(执行推理)，根据请求对应的模型进行分流推理
        */
        DataStream<InferenceResponse> responses = predicted_requests.rebalance().keyBy(req -> req.getLlmModel().getModelName()).process(new KeyedInferenceProcessor()).name("Keyed Inference Operator");
