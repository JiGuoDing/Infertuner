...
import com.infertuner.processor.MultiGPUInferenceProcessor;
import com.infertuner.sink.MultiGPUMonitorSink;
 ...
 * 多GPU推理作业
 */
public class MultiGPUInferenceJob {
    
    private static final Logger logger = LoggerFactory.getLogger(MultiGPUInferenceJob.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<InferenceRequest> requests = env
            .addSource(new SimpleRequestSource(maxRequests, interval, false))
            .name("Request Source");
        
        DataStream<InferenceResponse> responses = requests
            .map(new MultiGPUInferenceProcessor())  // 使用多GPU处理器
            .name("Multi-GPU Inference Processor");
        
        responses.addSink(new MultiGPUMonitorSink())
