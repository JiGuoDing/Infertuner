 ...
 * 通过设置不同的等待时间来模拟攒批效果
 */
public class SimpleRealBatchProcessor extends RichMapFunction<InferenceRequest, InferenceResponse> {
    
    private static final Logger logger = LoggerFactory.getLogger(SimpleRealBatchProcessor.class);
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        logger.info("Task {} 启动，绑定到 GPU {}，batch_size={}", taskIndex, gpuId, batchSize);
        
        objectMapper = new ObjectMapper();
        startInferenceService();
        
        logger.info("GPU {} 简化攒批推理服务启动完成", gpuId);
    }
    ...
    public InferenceResponse map(InferenceRequest request) throws Exception {
        ...
        long waitTime = 0;
        if (batchSize > 1) {
            waitTime = (batchSize - positionInBatch) * 200;  // 每个位置差200ms
        }
        
        logger.info("GPU {} 收到请求 {} (第{}个，批次位置{}), 模拟等待{}ms", 
                   gpuId, request.requestId, requestCounter, positionInBatch, waitTime);
